...
\label{sect:objectives}
 ...
 This Acceptance Test campaign aims to verify a subset of DM subsystem
requirements (\citeds{LSE-61}) related to the LSST Science Pipelines. It will be
executed in conjunction with the release of Science Pipelines Version
20.0.0, but the pipeline release is not contingent upon this test
campaign. This Test Plan aims to demonstrate that the included
requirements have been met by Version 20.0.0 of the Pipelines, and to
thus fully verify their completion and readiness for LSST commissioning.
...
dataset (as described in \citeds{DMTN-091}: Test Datasets for Scientific
Performance Monitoring). When possible, we will start our tests with the
data products resulting from processing HSC-RC2 with the v20\_0\_0\_rc1
pipelines release candidate (DM-24478) that was used to create v20 of
...
The current status of test plan \href{https://jira.lsstcorp.org/secure/Tests.jspa\#/testPlan/LVV-P71}{LVV-P71} in Jira is \textbf{ Completed }.
...
\multicolumn{2}{p{10.9cm}}{\textbf{ Science Pipelines Release 20.0.0 Acceptance Test Campaign }} & Completed \\\hline
\multicolumn{2}{r}{ T. Cycle \href{https://jira.lsstcorp.org/secure/Tests.jspa\#/testCycle/LVV-C153}{LVV-C153}:} &
\multicolumn{2}{p{10.9cm}}{\textbf{ Pipelines v20 Release DM Acceptance Test Campaign }} & Done \\\hline
...
\begin{minipage}[]{9cm}
\smallskip
This test demonstrates that the injected fake sources are visible, and
that their measured magnitudes match the input values. However, future
exploration should probably test the same for a variety of artificial
sources (i.e., including galaxies).
\medskip
\end{minipage}
&   \\\hline
\href{https://jira.lsstcorp.org/secure/Tests.jspa#/testCase/LVV-T36}{LVV-T36}
...
\begin{minipage}[]{9cm}
\smallskip
In this test, we have used precursor data from DECam for verification,
but any suitable precursor data would be sufficient. When ComCam data
become available, this test should be executed on them.
\medskip
\end{minipage}
&   \\\hline
\href{https://jira.lsstcorp.org/secure/Tests.jspa#/testCase/LVV-T42}{LVV-T42}
...
\begin{minipage}[]{9cm}
\smallskip
After some discussion, it was decided to defer this test to a later
campaign. This is due in part to the lack of on-site personnel at some
sites to execute the test. More importantly, however, is the need to
understand whether this should be divided into multiple tests that
address hardware at each of the various sites that are part of the DMS,
and whether those systems should all be taken down simultaneously, or
tested separately. We hope to execute this test as part of the next
...
\begin{minipage}[]{9cm}
\smallskip
The Gen3 Butler rollout was delayed, so this test was not executed.
Because many aspects of task specification and execution are in active
development, we will defer this test to a later campaign that takes
place after Gen3 has stabilized.
\medskip
\end{minipage}
&   \\\hline
\href{https://jira.lsstcorp.org/secure/Tests.jspa#/testCase/LVV-T145}{LVV-T145}
...
\smallskip
The Gen3 Butler rollout was delayed, so this test was not executed.
Because many aspects of task configuration, specification, and execution
are in active development, we will defer this test to a later campaign
that takes place after Gen3 has stabilized.
\medskip
\end{minipage}
&   \\\hline
\href{https://jira.lsstcorp.org/secure/Tests.jspa#/testCase/LVV-T45}{LVV-T45}
...
\begin{minipage}[]{9cm}
\smallskip
This test was included because it was anticipated that the
infrastructure to produce the Prompt Processing Data Quality Report
would be available by the time of this Test Campaign. Because the report
has not yet been defined, nor the automated pipeline to ingest and
process data, then generate the report, we defer this test to a later
campaign.
\medskip
\end{minipage}
&   \\\hline
...
\smallskip
This test was included because it was anticipated that the
infrastructure to produce the Prompt Processing Performance Report would
be available by the time of this Test Campaign. Because the report has
not yet been defined, nor the automated pipeline to ingest and process
data, then generate the report, we defer this test to a later campaign.
\medskip
\end{minipage}
&   \\\hline
...
datasets). A Jira issue for this work has been created
(DM-27586).\\[2\baselineskip]Testing of requirements related to
...
accuracy with which their properties can be recovered. A Jira issue to
do these updates has been created (DM-27587).\\[2\baselineskip]Finally,
this testing highlighted the need to clarify some requirements, or at
least to be explicit and clear about how they are being interpreted when
executing tests.
...
This test cycle verifies a subset of DM subsystem requirements (\citeds{LSE-61})
requirements related to the LSST Science Pipelines, in order to verify
their completion and readiness for LSST commissioning (i.e., that the
requirements laid out in \citeds{LSE-61} have been met by the DM Systems).
...
pipelines are still in development, the values for these metrics are not
the focus of this test; the actual performance will be verified during
commissioning on LSST data using this code. The requirement to be
satisfied is that we have (a) provided the software to calculate them,
and (b) provided a framework for comparing them to requirements. These
are both satisfied.
...
satisfied is that we have (a) provided the software to calculate them,
and (b) provided a framework for comparing them to requirements. These
are both satisfied.\\[2\baselineskip]This step is given a status of
``Initial Pass'' because there are no u-band data to test on. We have
demonstrated that the capability is provided, but simply do not have
data to pass through the code.
...
Final comment:\\This test demonstrates that the injected fake sources are visible, and
that their measured magnitudes match the input values. However, future
exploration should probably test the same for a variety of artificial
sources (i.e., including galaxies).
...
\end{minipage} \\ \cdashline{2-2}
 ...
 & Actual Result \\
 & \begin{minipage}[t]{15cm}{\footnotesize
We executed this test on the regularly reprocessed HSC-RC2 dataset. It
is now part of routine processing to insert artificial sources into the
data.
...
\end{minipage} \\ \cdashline{2-2}
 ...
 & Actual Result \\
 & \begin{minipage}[t]{15cm}{\footnotesize
We extracted a single patch, and then selected only the fake sources
within that RA, Dec box.
...
\end{minipage} \\ \cdashline{2-2}
 ...
 & Actual Result \\
 & \begin{minipage}[t]{15cm}{\footnotesize
This step was performed prior to the test. We did, however, load the
fakes catalog from the butler repository and examined them
using:\\[2\baselineskip]fakeSourceCat =
butler\_mb.get('deepCoadd\_fakeSourceCat', dataId = dataIdCoadd)
...
\end{minipage} \\ \cdashline{2-2}
 ...
 & Actual Result \\
 & \begin{minipage}[t]{15cm}{\footnotesize
We confirmed that the number of sources in the `forced\_src` photometry
catalog~\emph{including fakes} is larger than the catalog without fakes.
...
\end{minipage} \\ \cdashline{2-2}
 ...
 & Actual Result \\
 & \begin{minipage}[t]{15cm}{\footnotesize
See the associated notebook, test\_LVV-T125.ipynb, which shows
comparisons of images with and without fake sources, and differences of
images (to highlight the fake sources that are left after subtracting
...
\end{minipage} \\ \cdashline{2-2}
 ...
 & Actual Result \\
 & \begin{minipage}[t]{15cm}{\footnotesize
By extracting the detected/measured sources that correspond to the input
fakes, we compared the photometry and confirmed that the magnitudes of
input fake sources are recovered.
...
\end{minipage} \\ \cdashline{2-2}
 ...
 & Actual Result \\
 & \begin{minipage}[t]{15cm}{\footnotesize
We will use HSC-RC2 data that was processed with the v20.0 science
pipelines. The output repo is tagged with the Jira ticket number
``DM-25349'':\\
rc2\_repo =
`/datasets/hsc/repo/rerun/RC/v20\_0\_0\_rc1/DM-25349-sfm'\\[2\baselineskip]We
also repeat each test on data from the difference image repo at the
following path on the `lsst-devl` servers:\\
diffim\_repo = `/project/mrawls/cosmos/rerun/ap\_pipe-g-202007'
...
\end{minipage} \\ \cdashline{2-2}
 ...
 & Actual Result \\
 & \begin{minipage}[t]{15cm}{\footnotesize
butler = dafPersist.Butler(rc2\_repo)\\[2\baselineskip]The data is
...
\end{minipage} \\ \cdashline{2-2}
 ...
 & Actual Result \\
 & \begin{minipage}[t]{15cm}{\footnotesize
In the notebook associated with this test -- `test\_LVV-T39.ipynb` -- we
extract the zeropoints, fluxes, and magnitudes, and confirm that the
conversion between fluxes and magnitudes is enabled, and that it
round-trips succesfully.
...
\end{minipage} \\ \cdashline{2-2}
 ...
 & Actual Result \\
 & \begin{minipage}[t]{15cm}{\footnotesize
We extracted fluxes for 500 sources at random, and confirmed that
converting their fluxes to magnitudes yields reasonable values for all
stars.
...
Final comment:\\In this test, we have used precursor data from DECam for verification,
but any suitable precursor data would be sufficient. When ComCam data
become available, this test should be executed on them.
...
\end{minipage} \\ \cdashline{2-2}
 ...
 & \begin{minipage}[t]{15cm}{\footnotesize
The test was executed on the Science Platform Notebook Aspect with
version 20.0.0 set up.\\[2\baselineskip]These tests were executed using
processed data from DECam/HiTS. The processing with the Alert Production
payload was executed prior to the start of this testing. The path to the
data are given in the accompanying Jupyter notebook entitled
`test-T38\_42.ipynb`.
...
\end{minipage} \\ \cdashline{2-2}
 ...
 & Actual Result \\
 & \begin{minipage}[t]{15cm}{\footnotesize
The path where Meredith regularly re-runs HiTS data through diff
imaging:\\
diffim\_repo =
`/project/mrawls/hits2015/rerun/cw\_2020\_06'\\[2\baselineskip]butler =
dafPersist.Butler(diffim\_repo)
...
\end{minipage} \\ \cdashline{2-2}
 ...
 & Actual Result \\
 & \begin{minipage}[t]{15cm}{\footnotesize
In the associated notebook, we have examined a raw image and a processed
visit image (also known as a ``calexp'') side-by-side, and confirmed
that (a) the calexp is well-formed, and that (b) the instrumental
artifacts have been corrected.
...
\end{minipage} \\ \cdashline{2-2}
 ...
 & Actual Result \\
 & \begin{minipage}[t]{15cm}{\footnotesize
See the relevant code in the associated notebook. All of these objects
were selected as ``wcs'', ``calexp.image'', ``psf'', and ``src.''
...
\end{minipage} \\ \cdashline{2-2}
 ...
 & Actual Result \\
 & \begin{minipage}[t]{15cm}{\footnotesize
The images of both the calexp and its variance plane are well-formed. We
confirmed the presence of a mask plane and the meanings of its bit
values using afwDisplay within the Jupyter notebook.
...
\end{minipage} \\ \cdashline{2-2}
 ...
 & Actual Result \\
 & \begin{minipage}[t]{15cm}{\footnotesize
We extracted an image of the PSF at two random positions and noted that
they were well-formed. We then took the difference of the two PSFs,
which showed clear differences between them. We thus confirmed that the
PSF differs as a function of position.
...
\end{minipage} \\ \cdashline{2-2}
 ...
 & Actual Result \\
 & \begin{minipage}[t]{15cm}{\footnotesize
This was executed for all sources on a CCD in a processed visit image.
The XY positions converted to (RA, Dec) were confirmed to match the
expected position as given by the WCS.
...
\end{minipage} \\ \cdashline{2-2}
 ...
 & Actual Result \\
 & \begin{minipage}[t]{15cm}{\footnotesize
These steps were repeated for a difference image. Results of this
successful test can be seen in the associated notebook.
...
\end{minipage} \\ \cdashline{2-2}
 ...
 & Actual Result \\
 & \begin{minipage}[t]{15cm}{\footnotesize
ComCam data were obtained as part of the July 2020 Operations Rehearsal
(see the report in DMTR-231: ``LDM-503-11a: ComCam OPS Readiness Test
Plan and Report'').~
...
\end{minipage} \\ \cdashline{2-2}
 ...
 & Actual Result \\
 & \begin{minipage}[t]{15cm}{\footnotesize
This was verified as part of LVV-T1934, which was a test to confirm that
the data were transferred and ingested successfully from the Base
Facility to NCSA.
...
\end{minipage} \\ \cdashline{2-2}
 ...
 & Actual Result \\
 & \begin{minipage}[t]{15cm}{\footnotesize
We used the data from July 29, 2020, at the following path:\\
repo\_path = `/project/shared/comCam/rerun/CALSET\_20200729\_1847\_v2/'
...
\end{minipage} \\ \cdashline{2-2}
 ...
 & \begin{minipage}[t]{15cm}{\footnotesize
repo\_path =
`/project/shared/comCam/rerun/CALSET\_20200729\_1847\_v2/'\\
butler = dafPersist.Butler(repo\_path)
...
\end{minipage} \\ \cdashline{2-2}
 ...
 & Actual Result \\
 & \begin{minipage}[t]{15cm}{\footnotesize
In the associated Jupyter notebook entitled `test\_LVV-T1264.ipynb`, we
loaded a bias and a flat image from the above repo. Each of these was
displayed to confirm that it was a well-formed image, and histograms of
the count values were plotted to show that the counts were as expected
(i.e., that the bias values are centered on roughly 0.0, and the flat
values on roughly 1.0).
...
Final comment:\\After some discussion, it was decided to defer this test to a later
campaign. This is due in part to the lack of on-site personnel at some
sites to execute the test. More importantly, however, is the need to
understand whether this should be divided into multiple tests that
address hardware at each of the various sites that are part of the DMS,
...
Final comment:\\The Gen3 Butler rollout was delayed, so this test was not executed.
Because many aspects of task specification and execution are in active
development, we will defer this test to a later campaign that takes
place after Gen3 has stabilized.
...
Final comment:\\This test was included because it was anticipated that the
infrastructure to produce the Prompt Processing Data Quality Report
would be available by the time of this Test Campaign. Because the report
has not yet been defined, nor the automated pipeline to ingest and
process data, then generate the report, we defer this test to a later
